...
const config = {
    ...
    type: 'backend',
    capabilities: ['Supabase', 'PostgreSQL', 'FastAPI', 'Python', 'Database Design', 'API Development', 'Authentication', 'Real-time Systems'],
...
async function getClaudeResponse(userMessage) {
    const systemPrompt = `You are a Backend Specialist AI agent with expertise in:
...
- Include the appropriate language identifier after the opening backticks (e.g., \`\`\`python, \`\`\`sql, \`\`\`javascript, \`\`\`bash, \`\`\`json)
...
Example:
\`\`\`python
# File: /opt/standup-sydney-mcp/server.py
