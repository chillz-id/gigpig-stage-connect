...
console.log('🔧 FIXING TRANSFORM ORDERS FIELD MAPPING...');
console.log('===========================================');
...
// The CORRECTED JavaScript code with proper field mapping
const correctedJavaScript = `// CORRECTED Transform Orders - Maps to ACTUAL Notion database fields
...
console.log('==========================');
console.log('✅ Changed "Event Name" title field → "Name" title field');
console.log('✅ Customer name now goes to title field (not event name)');
console.log('✅ Event name now goes to "Event Name" rich_text field');
console.log('✅ Added all missing fields: Event ID, Event Date & Time, etc.');
console.log('✅ Fixed field types to match Create Entry expectations');

// Update workflow metadata
workflow.updatedAt = new Date().toISOString();
workflow.name = 'Humanitix Historical Import - All Time (Fixed Field Mapping)';

// Save the corrected workflow
fs.writeFileSync('/root/agents/scripts/workflow_fixed.json', JSON.stringify([workflow], null, 2));

console.log('✅ Corrected workflow saved as workflow_fixed.json');
