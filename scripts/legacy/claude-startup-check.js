...
try {
  ...
  if (mcpConfig.mcpServers.supabase?.env?.SUPABASE_ACCESS_TOKEN) {
    const tokenRef = mcpConfig.mcpServers.supabase.env.SUPABASE_ACCESS_TOKEN;

    // Check if it's using proper environment variable reference
    if (tokenRef === '${SUPABASE_ACCESS_TOKEN}') {
      // Now check if the actual environment variable exists
      try {
        const envContent = readFileSync('/root/agents/.env', 'utf8');
        const tokenMatch = envContent.match(/^SUPABASE_ACCESS_TOKEN=(.+)$/m);
        ...
        criticalIssues.push('❌ Cannot read environment file to verify Supabase token');
      }
    } else if (tokenRef.includes('NEEDS_') || tokenRef.includes('PLACEHOLDER')) {
      criticalIssues.push('❌ Supabase Personal Access Token missing - MCP tools won\'t work!');
    } else {
      successChecks.push('✅ Supabase MCP token configured');
    }
...
try {
  ...
  const envContent = existsSync('/root/agents/.env') ? readFileSync('/root/agents/.env', 'utf8') : '';
