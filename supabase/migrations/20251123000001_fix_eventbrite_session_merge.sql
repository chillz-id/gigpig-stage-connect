-- =====================================================
-- Fix Eventbrite Session Auto-Merge
-- =====================================================
-- The session_financials view was deriving eventbrite_sessions from sessions_htx
-- but that table only contains Humanitix data.
-- This migration derives Eventbrite sessions from orders_eventbrite which
-- contains all the needed metadata (event_name, event_start_date, event_timezone).
...
-- Recreate session_financials with Eventbrite sessions derived from orders_eventbrite
...
eventbrite_session_data AS (
  -- Eventbrite sessions derived from orders_eventbrite (NEW!)
  -- Each unique session_source_id with event metadata from orders
...
auto_session_links AS (
  ...
  -- 1. Same timezone
  -- 2. Start time within 30 minutes
...
COMMENT ON VIEW session_financials IS
  'Aggregates Humanitix and Eventbrite session financials with auto-merge based on timezone, time (30 min window), and fuzzy name matching';
