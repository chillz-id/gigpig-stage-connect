-- Update Smart Event Matching to remove time proximity credits (30min, 1hr)
-- Keeps same day, within 24hr, and within 72hr scoring

-- Drop and recreate the match_external_event function
...
BEGIN
  ...
  WITH match_scores AS (
    SELECT 
      ...
      -- Date and time proximity score (0-30 points)
      -- Removed 30min and 1hr partial credits
      CASE 
...
COMMENT ON FUNCTION match_external_event IS 'Matches external events to Stand Up Sydney events using title, venue, date/time, and keyword scoring. Date/time matching: exact match (30pts), same day different time (20pts), within 24hr (15pts), within 72hr (10pts). No partial credit for close times (30min/1hr).';