-- Fix customers_crm_v 500 error by restoring all 43 fields with customer_segments array
-- This migration combines the complete field set with customer_segments aggregation
...
SELECT
  ...
  -- Customer segments array (NEW - aggregates all segment assignments)
  seg_data.customer_segments

FROM customer_profiles cp
LEFT JOIN customer_engagement_metrics cem ON cem.customer_id = cp.id
-- LATERAL join to aggregate customer segments into an array
LEFT JOIN LATERAL (
  SELECT
    COALESCE(
...
COMMENT ON VIEW customers_crm_v IS 'CRM-compatible view that bridges customer_profiles with the schema expected by React UI hooks. Aggregates data from customer_profiles, customer_emails, customer_phones, customer_addresses, customer_engagement_metrics, customer_orders_v, and segments. Returns all 43 fields including customer_segments array.';
