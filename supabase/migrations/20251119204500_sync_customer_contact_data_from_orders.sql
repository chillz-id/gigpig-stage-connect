-- Migration: Sync Customer Contact Data from Orders
-- Created: 2025-11-19
-- Updated: 2025-11-19 (Fixed CTE scoping and duplicate phone handling)
-- Purpose: Create function to sync phone numbers, addresses, and DOB from orders
--          to normalized customer contact tables (customer_phones, customer_addresses, customer_profiles)
--
-- Bug Fix: Phone, address, and DOB data exists in orders_htx but wasn't syncing
--          to customer contact tables, causing CRM to show incomplete customer data
--
-- Results (after backfill):
--   - 1,770 customers synced with phones from orders_sync
--   - 3 customers synced with addresses from orders_sync
--   - 0 customers with DOB (Humanitix doesn't capture this field)

-- ============================================================================
-- FUNCTION: Sync Contact Data from Orders
-- ============================================================================

CREATE OR REPLACE FUNCTION public.refresh_customer_contact_data()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
DECLARE
  v_now timestamptz := now();
BEGIN
  -- Set context for audit logging
  PERFORM set_config('app.change_context', 'cron', true);

  -- ============================================================================
  -- STEP 1: Sync Phone Numbers from orders_htx
  -- ============================================================================

  -- Get most recent non-null phone for each customer from orders
  WITH customer_phones_from_orders AS (
    SELECT DISTINCT ON (ce.customer_id)
      ce.customer_id,
      oh.mobile as phone_e164,
      oh.ordered_at as last_seen_at
    FROM orders_htx oh
    JOIN customer_emails ce ON LOWER(TRIM(ce.email)) = LOWER(TRIM(oh.purchaser_email))
      AND ce.source = 'orders_sync'
    WHERE oh.mobile IS NOT NULL
      AND TRIM(oh.mobile) != ''
    ORDER BY ce.customer_id, oh.ordered_at DESC
  )
  INSERT INTO customer_phones (
    customer_id,
    phone_e164,
    is_primary,
    source,
    first_seen_at,
    last_seen_at
  )
  SELECT
    customer_id,
    phone_e164,
    true, -- is_primary
    'orders_sync',
    v_now,
    last_seen_at
  FROM customer_phones_from_orders
  ON CONFLICT (phone_e164, source) DO UPDATE
  SET
    last_seen_at = GREATEST(customer_phones.last_seen_at, EXCLUDED.last_seen_at),
    customer_id = EXCLUDED.customer_id; -- Update customer linkage if phone moved

  -- ============================================================================
  -- STEP 2: Sync Addresses from orders_htx
  -- ============================================================================

  -- Get most recent complete address for each customer from orders
  WITH customer_addresses_from_orders AS (
    SELECT DISTINCT ON (ce.customer_id)
      ce.customer_id,
      oh.address_street as line_1,
      NULL::text as line_2, -- orders_htx doesn't have line_2
      oh.address_suburb as suburb,
      oh.address_city as city,
      oh.address_state as state,
      oh.address_postal_code as postcode,
      oh.address_country as country,
      oh.ordered_at as last_seen_at
    FROM orders_htx oh
    JOIN customer_emails ce ON LOWER(TRIM(ce.email)) = LOWER(TRIM(oh.purchaser_email))
      AND ce.source = 'orders_sync'
    WHERE (
      oh.address_street IS NOT NULL
      OR oh.address_suburb IS NOT NULL
      OR oh.address_city IS NOT NULL
      OR oh.address_state IS NOT NULL
      OR oh.address_postal_code IS NOT NULL
      OR oh.address_country IS NOT NULL
    )
    ORDER BY ce.customer_id, oh.ordered_at DESC
  )
  -- First, update existing addresses from orders_sync
  UPDATE customer_addresses ca
  SET
    line_1 = NULLIF(TRIM(new_addr.line_1), ''),
    suburb = NULLIF(TRIM(new_addr.suburb), ''),
    city = NULLIF(TRIM(new_addr.city), ''),
    state = NULLIF(TRIM(new_addr.state), ''),
    postcode = NULLIF(TRIM(new_addr.postcode), ''),
    country = NULLIF(TRIM(new_addr.country), ''),
    last_seen_at = new_addr.last_seen_at
  FROM customer_addresses_from_orders new_addr
  WHERE ca.customer_id = new_addr.customer_id
    AND ca.source = 'orders_sync'
    AND ca.is_primary = true;

  -- Then insert new addresses for customers who don't have one from orders_sync yet
  INSERT INTO customer_addresses (
    customer_id,
    label,
    line_1,
    line_2,
    suburb,
    city,
    state,
    postcode,
    country,
    is_primary,
    source,
    first_seen_at,
    last_seen_at
  )
  SELECT
    new_addr.customer_id,
    'Primary',
    NULLIF(TRIM(new_addr.line_1), ''),
    NULL,
    NULLIF(TRIM(new_addr.suburb), ''),
    NULLIF(TRIM(new_addr.city), ''),
    NULLIF(TRIM(new_addr.state), ''),
    NULLIF(TRIM(new_addr.postcode), ''),
    NULLIF(TRIM(new_addr.country), ''),
    true, -- is_primary
    'orders_sync',
    v_now,
    new_addr.last_seen_at
  FROM customer_addresses_from_orders new_addr
  WHERE NOT EXISTS (
    SELECT 1 FROM customer_addresses
    WHERE customer_id = new_addr.customer_id
      AND source = 'orders_sync'
      AND is_primary = true
  );

  -- ============================================================================
  -- STEP 3: Sync Date of Birth from orders_htx
  -- ============================================================================

  -- Update customer_profiles with most recent non-null DOB from orders
  WITH customer_dob_from_orders AS (
    SELECT DISTINCT ON (ce.customer_id)
      ce.customer_id,
      oh.purchaser_date_of_birth as date_of_birth
    FROM orders_htx oh
    JOIN customer_emails ce ON LOWER(TRIM(ce.email)) = LOWER(TRIM(oh.purchaser_email))
      AND ce.source = 'orders_sync'
    WHERE oh.purchaser_date_of_birth IS NOT NULL
    ORDER BY ce.customer_id, oh.ordered_at DESC
  )
  UPDATE customer_profiles cp
  SET
    date_of_birth = dob.date_of_birth,
    updated_at = v_now
  FROM customer_dob_from_orders dob
  WHERE cp.id = dob.customer_id
    AND (cp.date_of_birth IS NULL OR cp.date_of_birth IS DISTINCT FROM dob.date_of_birth);

  -- Reset context
  PERFORM set_config('app.change_context', '', true);
END;
$$;

COMMENT ON FUNCTION public.refresh_customer_contact_data() IS
'Syncs customer contact data (phone, address, DOB) from orders to normalized customer tables. Run periodically to keep CRM data up to date.';

-- ============================================================================
-- STEP 4: Add to refresh_customer_data() cron job
-- ============================================================================

CREATE OR REPLACE FUNCTION public.refresh_customer_data()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  PERFORM public.refresh_customer_engagement_metrics();
  PERFORM public.refresh_customer_marketing_export();
  PERFORM public.refresh_customer_comedians_seen();
  PERFORM public.refresh_customer_contact_data(); -- NEW: Sync contact data from orders

  -- Refresh activity timeline materialized view
  REFRESH MATERIALIZED VIEW CONCURRENTLY public.customer_activity_timeline;
END;
$$;

COMMENT ON FUNCTION public.refresh_customer_data() IS
'Master function that refreshes all customer data aggregations. Called by cron job every 15 minutes. Now includes contact data sync and activity timeline refresh.';

-- ============================================================================
-- STEP 5: Run initial backfill
-- ============================================================================

-- Run the sync function to backfill existing data
SELECT public.refresh_customer_contact_data();

-- Also refresh the activity timeline to pick up any missing activities
REFRESH MATERIALIZED VIEW public.customer_activity_timeline;
