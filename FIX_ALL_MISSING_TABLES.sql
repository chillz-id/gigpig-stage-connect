-- ============================================
-- FIX ALL MISSING TABLES FOR STAND UP SYDNEY
-- ============================================
-- This script creates ALL missing tables: tasks, flights, and tours
...
-- Also check if has_role function exists (needed for RLS policies)
...
-- Create ENUM types for task system (if they don't exist)
DO $$ BEGIN
    CREATE TYPE task_status AS ENUM ('pending', 'in_progress', 'completed', 'cancelled', 'blocked', 'reviewing');
EXCEPTION
...
DO $$ BEGIN
    CREATE TYPE task_category AS ENUM (
        'artist_management', 'event_planning', 'marketing', 'finance', 
        'logistics', 'technical', 'administrative', 'creative', 
...
DO $$ BEGIN
    CREATE TYPE reminder_type AS ENUM ('email', 'push', 'sms', 'in_app');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
CREATE TABLE IF NOT EXISTS public.tasks (
    ...
    comedian_id UUID REFERENCES public.profiles(id) ON DELETE CASCADE,
...
-- Create indexes for task tables
...
CREATE POLICY "Users can view their own tasks" ON public.tasks
    FOR SELECT USING (creator_id = auth.uid() OR assignee_id = auth.uid() OR auth.uid() = ANY(watchers));

CREATE POLICY "Users can create tasks" ON public.tasks
    FOR INSERT WITH CHECK (creator_id = auth.uid());

CREATE POLICY "Task creators and assignees can update" ON public.tasks
    FOR UPDATE USING (creator_id = auth.uid() OR assignee_id = auth.uid());
...
AND table_name IN ('tasks', 'flights', 'flight_bookings', 'tours', 'tour_stops')
ORDER BY table_name;

-- Show counts of missing vs existing
WITH required_tables AS (
    SELECT unnest(ARRAY[
        ...
        'tours', 'tour_stops', 'tour_participants', 'tour_itinerary'
    ]) as table_name
)
SELECT 
...
SELECT 'All missing tables should now be created! Run the verification check to confirm.' as message;