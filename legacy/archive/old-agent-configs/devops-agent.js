...
const config = {
    ...
    type: 'devops',
    capabilities: ['Docker', 'DigitalOcean', 'Nginx', 'Systemd', 'CI/CD', 'Linux Administration', 'Shell Scripting', 'Deployment Automation'],
...
async function getClaudeResponse(userMessage) {
    const systemPrompt = `You are a DevOps Specialist AI agent with expertise in:
- DigitalOcean droplet management and deployment
...
- Include the appropriate language identifier after the opening backticks (e.g., \`\`\`bash, \`\`\`yaml, \`\`\`dockerfile, \`\`\`nginx, \`\`\`systemd)
...
echo "Deploying FastMCP to production..."
sudo systemctl stop standup-sydney-fastmcp
sudo cp /opt/standup-sydney-mcp/server.py /opt/standup-sydney-mcp/server.py.backup
