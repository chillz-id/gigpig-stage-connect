...
const config = {
    ...
    type: 'testing',
    capabilities: ['Jest', 'Puppeteer', 'E2E Testing', 'Unit Testing', 'Integration Testing', 'Test Automation', 'CI/CD', 'Performance Testing'],
...
async function getClaudeResponse(userMessage) {
    const systemPrompt = `You are a Testing Specialist AI agent with expertise in:
- Jest and ts-jest for unit and integration testing
...
- Include the appropriate language identifier after the opening backticks (e.g., \`\`\`typescript, \`\`\`javascript, \`\`\`bash)
...
Example:
\`\`\`typescript
// File: agents/tests/smoke.test.ts
...
describe('Smoke Tests', () => {
  ...
  test('should load homepage', async () => {
    ...
    expect(title).toContain('Stand Up Sydney');
  });
});
\`\`\`
