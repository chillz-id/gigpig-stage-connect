...
export interface VisualArtistProfileFormData {
  specialties: string[];
  services: string[]; // ['photography', 'videography', 'both']
...
interface VisualArtistProfileFormProps {
  initialData?: Partial<VisualArtistProfileFormData>;
  onSubmit: (data: VisualArtistProfileFormData) => Promise<void>;
  onCancel?: () => void;
  submitLabel?: string;
}

/**
 * Visual Artist Profile Form Component
 *
 * Form for creating or editing visual artist profiles (photographers and videographers).
 * Required fields: specialties (at least one), services (at least one)
 * Optional fields: experience_years, portfolio_url, video_reel_url, rate_per_hour,
 *                  instagram_portfolio, youtube_channel
 */
export function VisualArtistProfileForm({
  initialData,
  onSubmit,
  onCancel,
  submitLabel = 'Save Profile'
}: VisualArtistProfileFormProps) {
  const [isLoading, setIsLoading] = useState(false);
  const [formData, setFormData] = useState<VisualArtistProfileFormData>({
  ...
  const handleInputChange = (field: keyof VisualArtistProfileFormData, value: string | number) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const validate = (): boolean => {
    ...
    if (formData.services.length === 0) {
      newErrors.services = 'Please select at least one service (Photography and/or Videography)';
    }
  ...
  // Suggested specialties combining both photography and videography
  ...
  const showVideographyFields = formData.services.includes('videography');

  return (
    <form onSubmit={handleSubmit}>
      <Card>
        <CardHeader>
          <CardTitle>Visual Artist Profile</CardTitle>
          <CardDescription>
            Set up your visual artist profile to get hired for photography and videography work at comedy events
          </CardDescription>
        </CardHeader>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={() => handleServiceToggle('photography')}
                  />
                  <label
                    htmlFor="photography"
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={() => handleServiceToggle('videography')}
                  />
                  <label
                    htmlFor="videography"
                    className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input
                ...
                placeholder="@your_portfolio_account"
              />
            </div>
