/**
 * EventBannerImageEditor - Event banner editor
 *
 * Uses Toast UI Image Editor for full editing capabilities:
 * - Crop with 16:9 aspect ratio (event banner format)
 * - Rotate and flip
 * - Draw, shapes, text annotations
...
interface LegacyEventBannerImageEditorProps {
  isOpen: boolean;
  onClose: () => void;
...
interface ModernEventBannerImageEditorProps {
  isOpen: boolean;
  onClose: () => void;
...
type EventBannerImageEditorProps = LegacyEventBannerImageEditorProps | ModernEventBannerImageEditorProps;

// Type guard to check which interface is being used
function isModernProps(props: EventBannerImageEditorProps): props is ModernEventBannerImageEditorProps {
  return 'onImageSave' in props;
}

export const EventBannerImageEditor: React.FC<EventBannerImageEditorProps> = (props) => {
    ...
    async (savedImageData: SavedImageData, _designState: DesignState) => {
      ...
      try {
        ...
        if (isModernProps(props)) {
          ...
          console.warn(
            'EventBannerImageEditor: Using legacy onSave interface. Consider migrating to onImageSave for better functionality.'
          );
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      ...
      >
        <DialogHeader className="px-6 py-4 border-b border-slate-700">
          <DialogTitle className="text-white">Edit Event Banner</DialogTitle>
        </DialogHeader>
...
export default EventBannerImageEditor;
