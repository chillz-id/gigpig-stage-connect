...
interface EventBannerImageEditorProps {
  isOpen: boolean;
  onClose: () => void;
...
export const EventBannerImageEditor: React.FC<EventBannerImageEditorProps> = ({
  isOpen,
  onClose,
  onSave,
  imageUrl,
  ...
  const calculateBounds = useCallback(() => {
    ...
    const containerHeight = 450; // 8:3 aspect ratio for event banners
  ...
  const drawCanvas = useCallback(() => {
    ...
    // Event banner dimensions (8:3 aspect ratio)
    const containerWidth = 1200;
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-6xl bg-gray-800 border-gray-600 text-white">
        <DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-muted rounded-lg p-3">
            <p className="text-sm text-gray-300">
              Drag to reposition the banner. The dashed outline shows the final crop area. Image is locked to cover the full banner area (1200×450px).
            </p>
          </div>
